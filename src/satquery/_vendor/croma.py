# Vendored from antofuller/CROMA, MIT. See CROMA_LICENSE and provenance.json.
...
class PretrainedCROMA(nn.Module):
    def __init__(self, pretrained_path='CROMA_base.pt', size='base', modality='both', image_resolution=120):
        ...
        if modality in ['SAR', 'both']:
            ...
            # load weights
            self.s1_encoder.load_state_dict(checkpoint['s1_encoder'])
            self.GAP_FFN_s1.load_state_dict(checkpoint['s1_GAP_FFN'])
        ...
        if modality in ['optical', 'both']:
            ...
            # load weights
            self.s2_encoder.load_state_dict(checkpoint['s2_encoder'])
            self.GAP_FFN_s2.load_state_dict(checkpoint['s2_GAP_FFN'])
